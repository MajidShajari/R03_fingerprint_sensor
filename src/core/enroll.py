...
from src.core.status import EnrollStatus
...
class Enroll(FingerprintSensor):
    ...
    def notify(self, status: EnrollStatus):
        local_logger.info("Status: %s", status.name)
        self.on_status(status)

    def _enroll(self):
        self.notify(EnrollStatus.START)
        for _buffer in range(1, 3):
            if _buffer == 1:
                self.notify(EnrollStatus.PLACE_FINGER)
                if self._capture(timeout=CAPTURE_TIME_OUT, buffer=_buffer):
                    self.notify(EnrollStatus.REMOVE_FINGER)
            ...
            self.notify(EnrollStatus.PLACE_SAME_FINGER)
    ...
    def _check_template(self, location: int = -1) -> bool:
        ...
        if not location:
            self.notify(EnrollStatus.STORAGE_FULL)
            return EnrollStatus.STORAGE_FULL
        return location[0]
        ...
        if self._check_template():
            if location == -1:
                ...
                if location in self._sensor.templates:
                    ...
                    return EnrollStatus.LOCATION_OCCUPIED
